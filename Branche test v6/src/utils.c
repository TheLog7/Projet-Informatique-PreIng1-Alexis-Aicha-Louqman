...
void drawChoixPivot(UserData* pUserData, Screen* pScreen){
    setColor(4,255,255,0);
    setColorPair(4,4,0);
    if(pUserData->arrow == 1){
        drawText(pScreen, 77, 25, "->", 0);
    ...
    drawText(pScreen, 55, 13,   "|         |      |   |         |         |   |     |   |   \\ |   |        |                     |"  , 4);
    drawText(pScreen, 55, 14,   "|         |______|   |    _____|    _____|   |     |   |    \\|   |_____   |_____           _____|"  , 4);
    drawText(pScreen, 80, 25, "Taille de pivot : 3",      0);
    drawText(pScreen, 80, 27, "Taille de pivot : 5"    , 0);
}
...
void drawPlateau(UserData* pUserData, Screen* pScreen){
    ...
    move(10, 22);
    printw("le pivot fait du %dx%d.", pUserData->nb_Pivots, pUserData->nb_Pivots);
...
void drawPivot(UserData* pUserData, Screen* pScreen){
    setColor(2,255,0,0);
    setColorPair(9,2,0);
    drawLine(pScreen, 69, 11 + (4*pUserData->ypivot), 7*pUserData->nb_Pivots, '_' ,9);
    drawCol(pScreen, 13, 5, 59, '|' ,9);
    drawCol(pScreen, 200, 5, 59, '|' ,9);
    drawLine(pScreen, 13, 64, 188, '_' ,9);
}


void arrowSelectMenu(UserData* pUserData, Screen* pScreen, Event* pEvt){
...
void arrowSelectChoixNbJoueur(UserData* pUserData, Screen* pScreen, Event* pEvt){
    if(pEvt->code == KEY_ARROW_UP || pEvt->code == KEY_ARROW_DOWN){
                if (pUserData->arrow == 1){ // mets la fleche sur "lancer une nouvelle partie"
                    pUserData->arrow = 2;
                }
        ...
        if(pUserData->arrow == 1){ // si entrée sur "2joueurs"
            ...
            pUserData->page_Actuelle = CHOIX_PIVOT;
...
void arrowSelectChoixPivot(UserData* pUserData, Screen* pScreen, Event* pEvt){
    if(pEvt->code == KEY_ARROW_UP || pEvt->code == KEY_ARROW_DOWN){
                if (pUserData->arrow == 1){ // mets la fleche sur "lancer une nouvelle partie"
                    pUserData->arrow = 2;
                }
...
void arrowSelectChoixPlateau (UserData* pUserData, Screen* pScreen, Event* pEvt){
...
void arrowSelectPlateau(UserData* pUserData, Screen* pScreen, Event* pEvt){
    switch(pUserData->format){
    ...
    // au tour du jour suivant, a mettre dans le futur dans la derniere des fonctions du case PLATEAU dans event de test.c
    switch (pUserData->player_state){
        case TOUR_JOUEUR1:
            if(pEvt->code == KEY_RETURN){ 
                placePawn(pUserData,pScreen, JOUEUR1);
            }    
            break;
        case TOUR_JOUEUR2:
            if(pEvt->code == KEY_RETURN){ 
                placePawn(pUserData,pScreen, JOUEUR2);
            }    
            break;
        case TOUR_JOUEUR3:
            if(pEvt->code == KEY_RETURN){ 
                placePawn(pUserData,pScreen, JOUEUR3);
            }    
            break;
    }
}

void placePawn(UserData* pUserData, Screen* pScreen, char joueur){
    int largeur = 0;
    int hauteur = 0;
    int ligne = 0;
    int err = 0;
...
void pivotChoice(UserData* pUserData, Screen* pScreen, Event* pEvt){
    int largeur = 0;
    int hauteur = 0;
    ...
    while(ligne >= 0){
        ...
        for(int i = 0; i < hauteur*largeur; i++){
            if(((pUserData->plateau)[3*i].type == VIDE) && ((pUserData->plateau)[3*i].abscisse == (pUserData->arrow)-1) && ((pUserData->plateau)[3*i].ordonne == ligne)){               
                (pUserData->plateau)[3*i].type = joueur;
                i = largeur*hauteur;
                ligne = -1;
            }
        }
...
/*void pivot(UserData* pUserData, Screen* pScreen, Event* pEvt){
    int largeur = 0;
    int hauteur = 0;
    int case_actuelle = 0;
