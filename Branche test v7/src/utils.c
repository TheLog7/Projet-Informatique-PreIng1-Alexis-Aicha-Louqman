...
void arrow_select_ChoixPivot(UserData* pUserData, Event* pEvt){
    if(pEvt->code == KEY_ARROW_UP || pEvt->code == KEY_ARROW_DOWN){
                if (pUserData->arrow == 1){ // mets la fleche sur "lancer une nouvelle partie"
                    pUserData->arrow = 2;
                }
...
void pivot_select_Plateau(UserData* pUserData, Event* pEvt){
    ...
    switch (pEvt->code) {
        ...
        case KEY_RETURN:
            pUserData->pivot = OFF;
            pUserData->nb_Tours ++;
            // METTRE ICI LA FONCTION DE PIVOT AU NIVEAU MATRICIEL
            break;
    }
}
...
void pivot(UserData* pUserData, Screen* pScreen, Event* pEvt){
    ...
    int case_actuelle = 0;
    int* tab = NULL;
    int i = pUserData->xPivot, j = pUserData->yPivot, k = 0;
    ...
    if(pUserData->sensPivot == ANTIHORAIRE){
        ...
        for (int l = 0; l < (pUserData->nb_Pivots)*(pUserData->nb_Pivots); l++){
            while (pUserData->plateau[3*case_actuelle].ordonne != j || pUserData->plateau[3*case_actuelle].abscisse != i){
                case_actuelle ++;
            }
            pUserData->plateau[3*case_actuelle].type = tab[k];
        } // Bon petit génie (c'est faux t'es du futur) gros pd, la faut que tu remette les trucs dans la matrice, 
    }                         // en oubliant pas que ton ancre est en haut à gauche, et que tu vas afficher en colonne droite to gauche en remontant, 
                              // si tu réussis, y Alexis qui te suce, je veux savoir que t'as reussi bg sur ce, force en PHYSIQUE avec BOUMZI
    /*else {
