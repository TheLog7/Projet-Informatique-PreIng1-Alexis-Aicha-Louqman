...
void equality(UserData* pUserData){
    int largeur = 0;
    int hauteur = 0;
    int compteur = 0;
...
void createSave(UserData* pUserData){
    pUserData->win = 1;
    ...
    fwrite(pUserData->plateau, sizeof(Case), 48, pUserData->sauvegarde);
    fclose(pUserData->sauvegarde);
}
...
void restaureSave(UserData* pUserData) {
    ...
    fclose(pUserData->sauvegarde);  // Fermer le fichier avant la réouverture
    ...
    pUserData->sauvegarde = fopen("sauvegarde", "rb");  // Rouvrir le fichier en mode lecture binaire
    if (pUserData->sauvegarde == NULL) {
        RAGE_QUIT(73, "Erreur lors de la réouverture du fichier");
    }

    // Se déplacer vers la position où se trouve le plateau dans le fichier binaire
    fseek(pUserData->sauvegarde, sizeof(UserData), SEEK_SET);
    if (fread(pUserData->plateau, sizeof(Case), 48, pUserData->sauvegarde) != 48) {
