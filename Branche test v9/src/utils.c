...
void drawPlateau(UserData* pUserData, Screen* pScreen){
    ...
    if (pUserData->win && (pUserData->win != 3 && pUserData->win != 7 && pUserData->win != 8 && pUserData->win != 9)){
        drawText(pScreen, 70, 45, "              ___    ___    ___         ___     ___ ", 0);
        drawText(pScreen, 70, 46, "\\    /   |   |        |    |   |   |   |   |   |    ", 0);
        drawText(pScreen, 70, 47, " \\  /    |   |        |    |   |   |   |___|   |--- ", 0);
        drawText(pScreen, 70, 48, "  \\/     |   |___     |    |___|   |   |   \\   |___", 0);
    }
...
void init_plateau(UserData* pUserData){
    ...
    fwrite(pUserData->plateau, sizeof(Case), pUserData->allPions, pUserData->sauvegarde);
...
void restaureSave(UserData* pUserData){
    UserData* save = malloc(sizeof(UserData));
    ...
    if (fread(save, sizeof(UserData), 1, pUserData->sauvegarde) != 1) {
        RAGE_QUIT(72, "Probleme de lecture");
    }

    save->plateau = malloc(sizeof(Case)* save->allPions);
    if (save->plateau == NULL) {
        RAGE_QUIT(70, "Pas de plateau");
    }
    fread(save->plateau, sizeof (Case), save->allPions, pUserData->sauvegarde);

    save->sauvegarde = pUserData->sauvegarde;
    pUserData = save;
}