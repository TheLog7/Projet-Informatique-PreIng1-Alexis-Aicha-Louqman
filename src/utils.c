...
void drawMenu(UserData* pUserData, Screen* pScreen){ // Affiche la page du menu
...
void drawChoixNbJoueurs(UserData* pUserData, Screen* pScreen){ // Affiche la page de choix de nombre de joueurs
    setColor(4,255,255,0);
    setColorPair(4,4,0);
    if(pUserData->arrow == 1){
        drawText(pScreen, 77, 25, "->", 0);
...
void drawChoixSensPivot(UserData* pUserData, Screen* pScreen){ //Affiche la page de choix de sens du pivot
    setColor(4,255,255,0);
    setColorPair(4,4,0);
    if(pUserData->arrow == 1){
        drawText(pScreen, 77, 25, "->", 0);
...
void drawChoix_plateau(UserData* pUserData, Screen* pScreen){ // Affiche la page de choix de taille du plateau
    setColor(4,255,255,0);
    setColorPair(4,4,0);
...
void drawPlateau(UserData* pUserData, Screen* pScreen){ //Affiche le plateau de jeu
    int hauteur = 0;
    int largeur = 0;
    int case_actuelle = 0;
    ...
    if (pUserData->win == 4 || pUserData->win == 2 || pUserData->win == 12){
        drawText(pScreen, 70, 50, "              ___    ___    ___         ___     ___ ", 0);
        drawText(pScreen, 70, 51, "\\    /   |   |        |    |   |   |   |   |   |    ", 0);
        drawText(pScreen, 70, 52, " \\  /    |   |        |    |   |   |   |___|   |--- ", 0);
        drawText(pScreen, 70, 53, "  \\/     |   |___     |    |___|   |   |  \\    |___", 0);
        switch (pUserData->win) {
            case 4:
                drawText(pScreen, 70, 56, "(du joueur 2)", 0);
                break;
            case 12:
                drawText(pScreen, 70, 56, "(du joueur 3)", 0);
                break;
            case 2:
                drawText(pScreen, 70, 56, "(du joueur 1)", 0);


                break;
        }
    }
    else if (pUserData->win == 6 || pUserData->win == 16 || pUserData->win == 14 || pUserData->win == 18){
        drawText(pScreen, 70, 50, "EGALITE", 0);
    }
...
void init_plateau(UserData* pUserData){ //Initialise la matrice de cases, permettant de gérer le plateau de jeu
    int hauteur = 0;
    int largeur = 0;
    int case_actuelle = 0;
...
void drawSquare(Screen* pScreen, int x, int y, char type){ //Affiche les différents carrés dans chaque case
    switch(type){
        case IMMO:
...
void drawPivot(UserData* pUserData, Screen* pScreen){ //Affiche le pivot
    setColorPair(9,2,0);
...
void arrow_select_Menu(UserData* pUserData, Event* pEvt){ //Gère les touches pour la flèche dans le menu principal
    if(pEvt->code == KEY_ARROW_UP || pEvt->code == KEY_ARROW_DOWN){
        ...
        if(pUserData->arrow == 2){ // si entrée sur "continuer la partie"
            clear();
            restaureSave(pUserData);
        }
    }
}

void arrow_select_ChoixNbJoueur(UserData* pUserData, Event* pEvt){ //Gère les touches pour la flèche dans le menu pour le nombre de joueurs
    if(pEvt->code == KEY_ARROW_UP || pEvt->code == KEY_ARROW_DOWN){
                if (pUserData->arrow == 1){ // mets la fleche sur "lancer une nouvelle partie"
                    pUserData->arrow = 2;
                }
...
void arrow_select_ChoixSensPivot(UserData* pUserData, Event* pEvt){ //Gère les touches pour la flèche dans le menu de choix pour le sens du pivot.
    if(pEvt->code == KEY_ARROW_UP || pEvt->code == KEY_ARROW_DOWN){
        if (pUserData->arrow == 1){ // mets la fleche sur "lancer une nouvelle partie"
            pUserData->arrow = 2;
        }
...
void arrow_select_ChoixPlateau (UserData* pUserData, Event* pEvt){ //Gère les touches pour la flèche dans le menu de choix de la taille du plateau
...
void arrow_select_Plateau(UserData* pUserData, Event* pEvt){ //Gère les touches pour la flèche dans le plateau
    switch(pUserData->format){
        case HUITxSIX :
    ...
    if(pEvt->code == KEY_s){
        createSave(pUserData);
    }

}

void pivot_select_Plateau(UserData* pUserData, Event* pEvt, Screen* pScreen){ //Gère les touches pour le pivot
    int largeur = 0;
    int hauteur = 0;
    switch (pUserData->format){
        case HUITxSIX :
...
void placePawn(UserData* pUserData, char joueur){ //gère l'insertion de pions dans la matrice de pions
    int largeur = 0;
    int hauteur = 0;
    int ligne = 0;
    int err = 0;
...
void drawPivotOrArrow(UserData* pUserData, Screen* pScreen){ //décide de l'affichage du pivot ou de la flèche
    int largeur = 0;
...
void tourAntiHoraire(UserData* pUserData){ //permet la rotation de 90° dans le sens antihoraire, dans la matrice
    int case_actuelle = 0;
    int* tab = NULL;
...
void pivot(UserData* pUserData, Screen* pScreen){ //décide de faire le pivot dans le sens horaire ou antihoraire
    ...
    winCond(pUserData,pScreen);
}

void gravity(UserData* pUserData){ //s'occupe de la gravité après un pivot
    int hauteur = 0;
    int largeur = 0;
    int case_actuelle = 0;
...
int testHaut(UserData* pUserData, Screen* pScreen, int x, int y, int type, int nb){ // test si les pions du haut sont alignés
    int case_actuelle = 0;
    if (nb == 4){
        pUserData->win += type;
        return 1;
    ...
    if (y != 0 && type != VIDE){
        while (pUserData->plateau[3*case_actuelle].ordonne != y - 1 || pUserData->plateau[3*case_actuelle].abscisse != x){
            case_actuelle ++;
        }
        if (pUserData->plateau[3*case_actuelle].type == type){
            testHaut(pUserData,pScreen,x, y - 1, type, nb+1);
            return 0;
        }
        else{
            return 0;
...
int testBas(UserData* pUserData, Screen* pScreen, int x, int y, int type, int nb){ // test si les pions du bas sont alignés
    int case_actuelle = 0;
    int hauteur = 0;
    switch (pUserData->format){
        case HUITxSIX :
    ...
    if (y != hauteur - 1 && type != VIDE){
        while (pUserData->plateau[3*case_actuelle].ordonne != y + 1 || pUserData->plateau[3*case_actuelle].abscisse != x){
            case_actuelle ++;
        }
        if (pUserData->plateau[3*case_actuelle].type == type){
            testBas(pUserData,pScreen,x, y + 1, type, nb+1);
            return 0;
        }
        else{
            return 0;
...
int testDroite(UserData* pUserData, Screen* pScreen, int x, int y, int type, int nb){ // test si les pions de droite sont alignés
    int case_actuelle = 0;
    int largeur = 0;
    switch (pUserData->format){
        case HUITxSIX :
    ...
    if (x != largeur - 1 && type != VIDE){
        while (pUserData->plateau[3*case_actuelle].ordonne != y || pUserData->plateau[3*case_actuelle].abscisse != x + 1){
            case_actuelle ++;
        }
        if (pUserData->plateau[3*case_actuelle].type == type){
            testDroite(pUserData,pScreen,x + 1, y, type, nb+1);
            return 0;
        }
        else{
            return 0;
...
int testGauche(UserData* pUserData, Screen* pScreen, int x, int y, int type, int nb){ // test si les pions de gauche sont alignés
    int case_actuelle = 0;
    if (nb == 4){
        pUserData->win += type;
        return 1;
    ...
    if (x != 0 && type != VIDE){
        ...
        if (pUserData->plateau[3*case_actuelle].type == type){
            testGauche(pUserData,pScreen,x - 1, y, type, nb+1);
            return 0;
        }
        else{
            return 0;
...
int testHautGauche(UserData* pUserData, Screen* pScreen, int x, int y, int type, int nb){ // test si les pions en haut à gauche sont alignés
    int case_actuelle = 0;
    if (nb == 4){
        pUserData->win += type;
        return 1;
    }
    if (x != 0 && y != 0 && type != VIDE){
        while (pUserData->plateau[3*case_actuelle].ordonne != y - 1|| pUserData->plateau[3*case_actuelle].abscisse != x - 1){
            case_actuelle ++;
        }
        if (pUserData->plateau[3*case_actuelle].type == type){
            testHautGauche(pUserData,pScreen,x - 1, y - 1, type, nb+1);
            return 0;
        }
        else{
            return 0;
...
int testBasGauche(UserData* pUserData, Screen* pScreen, int x, int y, int type, int nb){ // test si les pions en bas à gauche sont alignés
    int case_actuelle = 0;
    int hauteur = 0;
    switch (pUserData->format){
        case HUITxSIX :
    ...
    if (y != hauteur - 1 && x != 0 && type != VIDE){
        while (pUserData->plateau[3*case_actuelle].ordonne != y + 1 || pUserData->plateau[3*case_actuelle].abscisse != x - 1){
            case_actuelle ++;
        }
        if (pUserData->plateau[3*case_actuelle].type == type){
            testBasGauche(pUserData,pScreen,x - 1, y + 1, type, nb+1);
            return 0;
        }
        else{
            return 0;
...
int testHautDroite(UserData* pUserData, Screen* pScreen, int x, int y, int type, int nb){ // test si les pions en haut à droite sont alignés
    int case_actuelle = 0;
    int largeur = 0;
    switch (pUserData->format){
        case HUITxSIX :
    ...
    if (x != largeur - 1 && y != 0 && type != VIDE){
        while (pUserData->plateau[3*case_actuelle].ordonne != y - 1 || pUserData->plateau[3*case_actuelle].abscisse != x + 1){
            case_actuelle ++;
        }
        if (pUserData->plateau[3*case_actuelle].type == type){
            testHautDroite(pUserData,pScreen,x + 1, y - 1, type, nb+1);
            return 0;
        }
        else{
            return 0;
...
int testBasDroite(UserData* pUserData, Screen* pScreen, int x, int y, int type, int nb){ // test si les pions en bas à droite sont alignés
    int hauteur = 0;
    int largeur = 0;
    int case_actuelle = 0;
    switch (pUserData->format){
    ...
    if (x != largeur - 1 && y != hauteur - 1 && type != VIDE){
        while (pUserData->plateau[3*case_actuelle].ordonne != y + 1 || pUserData->plateau[3*case_actuelle].abscisse != x + 1){
            case_actuelle ++;
        }
        if (pUserData->plateau[3*case_actuelle].type == type){
            testBasDroite(pUserData,pScreen,x + 1, y + 1, type, nb+1);
            return 0;
        }
        else{
            return 0;
...
void winCond(UserData* pUserData, Screen* pScreen){ // teste la condition de victoire (après le pivot)
    int hauteur = 0;
    int largeur = 0;
    switch (pUserData->format){
        case HUITxSIX :
    ...
    for (int i = 0; i < largeur * hauteur; i ++){
        testHaut(pUserData,pScreen,pUserData->plateau[3*i].abscisse,pUserData->plateau[3*i].ordonne,pUserData->plateau[3*i].type, 0);
        testBas(pUserData,pScreen,pUserData->plateau[3*i].abscisse,pUserData->plateau[3*i].ordonne,pUserData->plateau[3*i].type, 0);
        testGauche(pUserData,pScreen,pUserData->plateau[3*i].abscisse,pUserData->plateau[3*i].ordonne,pUserData->plateau[3*i].type, 0);
        testDroite(pUserData,pScreen,pUserData->plateau[3*i].abscisse,pUserData->plateau[3*i].ordonne,pUserData->plateau[3*i].type, 0);
        testHautGauche(pUserData,pScreen,pUserData->plateau[3*i].abscisse,pUserData->plateau[3*i].ordonne,pUserData->plateau[3*i].type, 0);
        testHautDroite(pUserData,pScreen,pUserData->plateau[3*i].abscisse,pUserData->plateau[3*i].ordonne,pUserData->plateau[3*i].type, 0);
        testBasGauche(pUserData,pScreen,pUserData->plateau[3*i].abscisse,pUserData->plateau[3*i].ordonne,pUserData->plateau[3*i].type, 0);
        testBasDroite(pUserData,pScreen,pUserData->plateau[3*i].abscisse,pUserData->plateau[3*i].ordonne,pUserData->plateau[3*i].type, 0);
    }

}

void createSave(UserData* pUserData){ //crée une sauvegarde
    move(15, 145);
    printw("Partie sauvegardée.");
    fclose(pUserData->sauvegarde);
    pUserData->sauvegarde = fopen("sauvegarde", "wb");
    fwrite(pUserData, sizeof(UserData), 1, pUserData->sauvegarde);
    fwrite(pUserData->plateau, 3 * sizeof(Case), 48, pUserData->sauvegarde);
    fclose(pUserData->sauvegarde);
}

void restaureSave(UserData* pUserData) { //ouvre la sauvegarde (plante s'il n'y en a pas)
    if (pUserData->sauvegarde == NULL) {
        RAGE_QUIT(71, "Pas de sauvegarde");
    }

    if (fread(pUserData, sizeof(UserData), 1, pUserData->sauvegarde) != 1) {
        RAGE_QUIT(72, "Probleme de lecture");
    }

    fclose(pUserData->sauvegarde);

    pUserData->plateau = malloc(sizeof(Case) * pUserData->allPions);
    if (pUserData->plateau == NULL) {
        RAGE_QUIT(70, "Pas de plateau");
    }

    pUserData->sauvegarde = fopen("sauvegarde", "rb");
    if (pUserData->sauvegarde == NULL) {
        RAGE_QUIT(73, "Erreur lors de la réouverture du fichier");
    }

    fseek(pUserData->sauvegarde, sizeof(UserData), SEEK_SET);
    if (fread(pUserData->plateau, 3 * sizeof(Case), 48, pUserData->sauvegarde) != 48) {
        RAGE_QUIT(72, "Probleme de lecture du plateau");
    }

    pUserData->win = 0;

    if (feof(pUserData->sauvegarde)) {
        fclose(pUserData->sauvegarde);
        RAGE_QUIT(69, "Pas de sauvegarde disponible");
    }
}
