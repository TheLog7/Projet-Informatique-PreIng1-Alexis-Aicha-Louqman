...
void drawPlateau(UserData* pUserData, Screen* pScreen){
    ...
    move(8, 22);
    printw("au tour du joueur: %d", pUserData->player_state);
    ...
    move(20,20);
    printw("type 2 : %d", (pUserData->plateau)[2].type);
    move(21,20);
    printw("type 3 : %d", (pUserData->plateau)[3].type);
    move(22,20);
    printw("type 4 : %d", (pUserData->plateau)[4].type);
    move(23,20);
    printw("type 6 : %d", (pUserData->plateau)[6].type);
    move(24,20);
    printw("type 1 : %d", (pUserData->plateau)[1].type);
    move(25,20);
    printw("type 0 : %d", (pUserData->plateau)[0].type);
...
void arrowSelectPlateau(UserData* pUserData, Screen* pScreen, Event* pEvt){
    ...
    if(pEvt->code == KEY_RETURN){ 
        placePawn(pUserData,pScreen);
    }    
}

void placePawn(UserData* pUserData, Screen* pScreen){
    int largeur = 0;
    int hauteur = 0;
    int j = 0;
    ...
    for (int i = 0; i < largeur*hauteur;i ++){
        ...
        if ((pUserData->plateau)[3*i].abscisse == (pUserData->arrow) - 1){
            while((pUserData->plateau)[3*j].type == VIDE && (pUserData->plateau)[3 * j].abscisse == (pUserData->arrow) - 1){
                j--;
            }
            (pUserData->plateau)[3*j].type = JOUEUR1;
        } 
    }
}
